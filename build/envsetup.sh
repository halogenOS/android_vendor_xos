function __print_xos_functions_help() {
cat <<EOF
Additional halogenOS functions:
...
function eat()
{
    ...
    if (adb shell getprop ro.xos.device | grep -q "$XOS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $XOS_BUILD, run away!"
    fi
}
...
function xosremote()
{
    ...
    git remote rm xosremote 2> /dev/null
    GERRIT_REMOTE=$(git config --get remote.github.projectname)
    XOSUSER=$(git config --get review.review.halogenos.org.username)
    if [ -z "$CMUSER" ]
    then
        git remote add xosremote ssh://review.halogenos.org:29418/$GERRIT_REMOTE
    else
        git remote add xosremote ssh://XOSUSER@review.halogenos.org:29418/$GERRIT_REMOTE
    fi
    echo "Remote 'xosremote' created"
}
...
function cafremote()
{
    ...
    git remote add caf https://source.codeaurora.org/$PFX$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        xosremote
        git push xosremote HEAD:refs/heads/'$1'
    fi
    '
}
...
EOF
            ...
            case $1 in
                __xosg_*) echo "For internal use only." ;;
                changes|for)
                    if [ "$FUNCNAME" = "xosgerrit" ]; then
...
EOF
                    ;;
                *)
                    $FUNCNAME __xosg_err_not_supported $1 && return
                    cat <<EOF
...
EOF
            ...
            ;;
        __xosg_get_ref)
            $FUNCNAME __xosg_err_no_arg $command $# && return 1
            ...
            ;;
        fetch|pull)
            $FUNCNAME __xosg_err_no_arg $command $# help && return 1
            $FUNCNAME __xosg_err_not_repo && return 1
            local change=$1
            shift
            git $command $@ http://$review/p/$project \
                $($FUNCNAME __xosg_get_ref $change) || return 1
            ;;
        push)
            $FUNCNAME __xosg_err_no_arg $command $# help && return 1
            $FUNCNAME __xosg_err_not_repo && return 1
        ...
        __xosg_err_not_repo)
            ...
            ;;
        __xosg_err_not_supported)
            $FUNCNAME __xosg_err_no_arg $command $# && return
            case $1 in
    ...
    #TODO: other special cases?
        *)
            $FUNCNAME __xosg_err_not_supported $command && return 1
            $FUNCNAME __xosg_err_no_arg $command $# help && return 1
            $FUNCNAME __xosg_err_not_repo && return 1
            local args="$@"
...
function dopush()
{
    ...
    if (adb shell getprop ro.xos.device | grep -q "$XOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
